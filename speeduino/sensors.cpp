...
#include "src/InternalTemperature/InternalTemperature.h"
...
void readBat(void)
{
  int16_t tempReading = fastMap10Bit(readAnalogSensor(pinBat), 0, 152); //Get the current raw Battery value. Permissible values are from 0v to 24.5v (245)
...
void readIdleTPS()
{
  currentStatus.idleTpsADC = readAnalogSensor(pinIdleTPS);

  if(configPage15.idleTpsMax > configPage9.idleTpsMin)
  {
    uint16_t tempADC = clamp(currentStatus.idleTpsADC, configPage9.idleTpsMin, configPage15.idleTpsMax);
    currentStatus.idleTPS = map(tempADC, configPage9.idleTpsMin, configPage15.idleTpsMax, 0, 100); 
  }
  else
  {
    uint16_t tempADC = UINT16_MAX - currentStatus.idleTpsADC; //Reverse the ADC values
    uint16_t tempTPSMax = UINT16_MAX - configPage15.idleTpsMax;
    uint16_t tempTPSMin = UINT16_MAX - configPage9.idleTpsMin;

    //All checks below are reversed from the standard case above
    tempADC = clamp(tempADC, tempTPSMin, tempTPSMax);
    currentStatus.idleTPS = map(tempADC, tempTPSMin, tempTPSMax, 0, 100);
  }
}
...
//Teensy 4.1 does not require .begin() to be called. This introduces a 700ms delay on startup time whilst USB is enumerated if it is called
    ...
    */
    ...
    BIT_CLEAR(currentStatus.status5, BIT_STATUS5_FUEL_PUMP);
  ...
  switch (boardID)
  {
   ...
   case 31:
    ...
    case 55:
      ...
      pinInjector4 = 1; //CS for MC33810 1
      pinInjector5 = 2; //CS for MC33810 2
      pinInjector6 = 2; //CS for MC33810 3
      ...
      */
      ...
      pinTrigger2 = 32; //The Cam Sensor pin
      pinTrigger3 = 20; //Uses one of the protected spare digitial inputs. This must be set or Serial1 (Pin 0) gets broken
      
      pinLaunch = 35; //Launch control input
      pinTachOut = 40; //Tacho output pin
      pinIdle1 = 3; //Single wire idle control
      pinIdle2 = 8; //2 wire idle control. Shared with Spare 1 output

      pinSpareLOut1 = 10; //low current output spare1
      pinSpareLOut2 = 10; //low current output spare2
      pinSpareLOut3 = 10; //low current output spare3
      pinSpareLOut4 = 10; //low current output spare4
      pinFuelPump = 6; //Fuel pump output
      pinFan = 4; //Pin for the fan output
      pinBoost = 5; //Boost control
      pinVVT_1 = 7; //Default VVT output
      pinStepperDir = 7; //Direction pin for DRV8825 driver
